...
The need for a new, precise language that is consistent with common usage, but makes crisp concepts. 

#### possibilities and events

we'll build all components of the model out of "events" as primitive for describing the world. e,.g. what we apply probabilities to.

What is the motivation for the idea of an event? 
Thinking of what has happened and what will or can happen - the quandary of infinities of futures. The finest grained "elemental outcomes" can be refined without limit. 

"Possible worlds" - 

Probability of what has happened == 1 Probability of what will happen = 0!

Thought experiment:  An proper distinction is so specific that an hypothetical "clairvoyant" would know, just based on the distinction whether the event occurred or not."

Resolve the quandary -- make distinctions. 

A **statement** or a **proposition** that is either true or false at a specific point in time. Events are defined by **distinctions**: a 'crisp' yes/no question about the world. Use the proposition to label the event. 

"Things" that haven't occurred are "variables"  So we encode variables by the distinctions that define them. 

In models we will identify three kinds of variables: uncertainties, decisions, and outcomes.  THe "three legged stool" of analyzing a decision. _Each variable is created from one or more distinctions.  Mutually exclusive and exhaustive.

Propositions can be combined by AND, OR, NOT to derive more events.  

Binary variable - simple (yes, no) distinction.  Compound distinction - multiple degrees. 
...
(Show example of a labeled event tree. Can we diagram the class deal? )
...
All modeling processes share similar methods. 

What's the point? 
    Best choice / Rational action / "Clarity of Action"  If the process is followed the tenets are achieved!  This is not guaranteed with "analysis" methods in general. 
...
An organization that acts like an individual decision maker is acting rationally. This means seeking better outcomes. 
...
We need a way to evaluate decisions.  if outcomes are not available - Decision Quality
