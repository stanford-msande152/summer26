...
Also traces the full-width re-extractions saved by step2_extract_figures.py
(input: <name>ex.png, output: <name>ex.svg).
"""
...
# Section 3 figures re-extracted at full page width by step2.
# These are traced as <name>ex.png -> <name>ex.svg.
FIGURES_FULLWIDTH = [
    "Figure3-04",
    "Figure3-05",
    "Figure3-06",
...
def trace(fig_name, png_suffix="", svg_suffix=""):
    """Trace <fig_name><png_suffix>.png -> <fig_name><svg_suffix>.svg."""
    png_path = INPUT_DIR / f"{fig_name}{png_suffix}.png"
    svg_path = OUTPUT_DIR / f"{fig_name}{svg_suffix}.svg"

    if not png_path.exists():
        print(f"  SKIP {fig_name}{png_suffix}: PNG not found")
        return False

    label = f"{fig_name}{svg_suffix}" if svg_suffix else fig_name
    print(f"  Tracing {label}...", end=" ", flush=True)
    try:
...
def main():
    ...
    fw_success = 0
    fw_skipped = 0
    print("\n== Full-width tracings (*ex.svg) ==")
    for fig_name in FIGURES_FULLWIDTH:
        if trace(fig_name, png_suffix="ex", svg_suffix="ex"):
            fw_success += 1
        else:
            fw_skipped += 1

    total = len(FIGURES) + len(FIGURES_FULLWIDTH)
    total_success = success + fw_success
    total_skipped = skipped + fw_skipped
    print(
        f"\nDone. {total_success}/{total} SVGs created "
        f"({success} standard, {fw_success} full-width), "
